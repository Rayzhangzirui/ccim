                  ...
                  GridData &grid)
{
   ...
   for (r = 0; r < 2*deg; r++)
   {
      rindex[rstar] = min(max(index[rstar]+sstar*(-deg+1+r),0),grid.nx[rstar]);
      temp[r] = evalarray(u,rindex);
   }
...
double regulafalsiweno(double ***u, int *index, int rstar, int sstar, GridData &grid)
{
   ...
   for (r = 0; r < 2*deg; r++)
   {
      rindex[rstar] = min(max(index[rstar]+sstar*(-deg+1+r),0),grid.nx[rstar]);//-2, -1, 0, 1, 2, 3
      temp[r] = evalarray(u,rindex);
   }
...
double regulafalsiweno(double *u, int i, int j, double a, double dx, double nx)
{
   ...
   for (step = 1; notstop; step++)
   {
      ...
      if (min(fabs(s-b),fabs(s-c)) < tol || us == 0.0)
         notstop = 0;
      else
         if (us*ub < 0.0)
         {
